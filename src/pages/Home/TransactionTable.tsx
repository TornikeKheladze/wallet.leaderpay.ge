...
const TransactionTable: React.FC<TransactionTableProps> = ({
    ...
    <tr className="lg:table-row hidden border-b border-buttonGray text-textBlack">
      {staticArr.map(({ filter, name, options }) => {
        if (filter === "date") {
          return (
            <th key={filter + name}>
              <div className="flex md:flex-row flex-col items-center gap-2 pl-2">
                <input
                  type="date"
                  onChange={(e) =>
  ...
  const onRowClick = (transaction: any) => {
    ...
    setActiveTransactions(transaction);
  };
        ...
        <thead>
          <tr>
            {staticArr.map(({ name }, index) => (
              ...
              >
                <div className="flex items-center justify-between gap-2 whitespace-nowrap text-primaryYellow">
                  <span>{t(name)}</span>
                </div>
              </th>
            ))}
              ...
              {staticArr.map(({ name, options }, columnIndex) => {
                ...
                return (
                  ...
                  >
                    {name === "amount" ? (
                      row.type === 1 ? (
                        <span className="text-green-500">+{row[name]}</span>
                      ) : (
                        <span className=" text-red-500">-{row[name]}</span>
                      )
                    ) : (
                      <span className="text-textPrimary">
                        {rowValue || row[name] || row.description}
                      </span>
                    )}
                  </td>
                );
